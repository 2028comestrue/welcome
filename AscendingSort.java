...
// Full Name: Hyeonmin Yang
// Campus ID: 9081998412
// WiscEmail: hyang486wisc.edu
// (TODO: fill this out)
////////////////////////////////////////////////////////////////////////////////

/**
 * This is a static, utility class containing methods to accomplish a recursive
 * sort of an array of Song objects. You do NOT have to implement the Song
...
public class AscendingSort {
	...
	/**
	 * A recursive helper method to find the smaller of: - the argument minSong, and
	 * - the shortest Song in the rest of playlist, from 0 to index (inclusive)
	 * 
	 * @param minSong  the minimum value over all indexes from index+1 to the end of
	 *                 the playlist
	 * @param playlist a full, perfect-size array of Songs with length >= 1
	 * @param index    the last index of the un-searched part of the playlist. valid
	 *                 values: -1 to playlist.length-2 (inclusive) If index == -1,
	 *                 the un-searched part of the playlist is empty.
	...
	private static Song getShortestSongHelper(Song minSong, Song[] playlist, int index) {
		...
		if (minSong.compareTo(playlist[index]) > 0) {
			minSong = playlist[index];
		}
		// 3. Return the shortest song
		return getShortestSongHelper(minSong, playlist, index - 1); // default return statement added to avoid compiler
																	// errors. Feel free to change it.
	}

	/**
	 * A tester method for getShortestSong, which you must complete. This method
	 * WILL be tested.
	 * 
	...
	public static boolean testGetShortestSong() {
		boolean test = true;
		// TODO
		// 4. Create a Song[] array containing at least 3 Song objects with different
		// titles and durations (see
		// testAscendingSort() below for examples)
		Song[] testPlaylist = new Song[] { new Song("Bejeweled", 3, 14), new Song("Snow on the Beach", 4, 16),
				new Song("Vigilante Shit", 2, 44), new Song("Question...?", 3, 30) };
		// 5. Create a NEW Song object that matches the Song in the array that has the
		// shortest duration
		// (e.g. for Songs with durations [5:32, 7:15, 4:56], you'd create a new Song
		// with duration 4:56).
		Song shortestSong = new Song("Vigilante Shit", 2, 44);
		// 6. Call getShortestSong() on your Song[] array and save the result
		Song testShortest = getShortestSong(testPlaylist);
		// 7. Compare this result against your expected value and return false if they
		// do not match.
		// CAREFUL: the equals() method is NOT overridden in the Song class. To find
		// equivalent Song
		// objects, use Song's compareTo() method.
		if (testShortest.compareTo(shortestSong) != 0) {
			test = false;
		}
		// OPTIONAL: implement additional tests to verify the method works!

		// 8. Return true if all tests in this method are passed.
		return test; // default return statement added to avoid compiler errors. Feel free to change
						// it.
	}
	...
	/**
	 * Returns a new array of size playlist.length-1 which contains every Song in
	 * playlist that is NOT the Song to remove. The method should NOT make any
	 * changes to the contents of playlist.
	 *
	 * Note that while there is no equals() method defined in Song, equal Songs will
	 * have a compareTo() value of 0.
	 * 
	 * @param playlist a full, perfect-size array of Songs with length >= 1
	 * @param toRemove the Song to remove from the playlist
	 * @return an array containing every Song from playlist EXCEPT toRemove, and NO
	 *         null values
	 */
	public static Song[] remove(Song[] playlist, Song toRemove) {
		...
		Song[] changed = new Song[playlist.length - 1];
		// 10. Iterate through the playlist and add all Songs that are NOT the Song
		// toRemove
		...
		for (int i = 0; i < minIndex; i++) {
			changed[i] = playlist[i];
		}
		for (int i = minIndex + 1; i < playlist.length; i++) {
			changed[i - 1] = playlist[i];
		}

		// 11. Return the resulting array
		return changed; // default return statement added to avoid compiler errors. Feel free to change
						// it.
	}
	...
	public static boolean testRemove() {
		boolean test = true;
		// TODO
		// 12. Create a Song[] array containing at least 3 Song objects with different
		// titles and durations (see
		// testAscendingSort() below for examples)
		Song[] testPlaylist = new Song[] { new Song("Bejeweled", 3, 14), new Song("Snow on the Beach", 4, 16),
				new Song("Vigilante Shit", 2, 44), new Song("Question...?", 3, 30) };
		// 13. Create a NEW Song object that is identical to one of the Song objects
		// from your array
		Song removedSong = new Song("Vigilante Shit", 2, 44);
		// 14. Call remove() on your array and Song object and save the result
		Song[] testRemoved = remove(testPlaylist, removedSong);
		...
		// test does
		// not pass.
		if (testRemoved.length != testPlaylist.length - 1) {
			test = false;
		}
		for (int i = 0; i < testRemoved.length; i++) {
			if (testRemoved[i].compareTo(removedSong) == 0) {
				test = false;
			}
		}
		// OPTIONAL: implement additional tests to verify the method works!
	...
	///////////////////////////////////////////////////////////////////////////////////////////////// your
	///////////////////////////////////////////////////////////////////////////////////////////////// program
	// works as expected, and a main method. The Song class is included at the
	///////////////////////////////////////////////////////////////////////////////////////////////// bottom
	///////////////////////////////////////////////////////////////////////////////////////////////// of
	///////////////////////////////////////////////////////////////////////////////////////////////// the
	///////////////////////////////////////////////////////////////////////////////////////////////// file.
	...
	/**
	 * A tester method for ascendingSort, provided for you in its entirety. You may
	...
	public static boolean testAscendingSort() {
		...
		if (simplePlaylist.length != 1)
			return false;
		if (simplePlaylist[0] == null || simplePlaylist[0].compareTo(contents) != 0)
			return false;

		// try sorting a playlist with multiple songs:
		Song[] testPlaylist = new Song[] { new Song("Bejeweled", 3, 14), new Song("Snow on the Beach", 4, 16),
				new Song("Vigilante Shit", 2, 44), new Song("Question...?", 3, 30) };
		Song[] expectedPlaylist = new Song[] { new Song("Vigilante Shit", 2, 44), new Song("Bejeweled", 3, 14),
				new Song("Question...?", 3, 30), new Song("Snow on the Beach", 4, 16) };
		try {
		...
		if (testPlaylist.length != expectedPlaylist.length)
			return false;
		for (int i = 0; i < expectedPlaylist.length; i++) {
			if (testPlaylist[i] == null || testPlaylist[i].compareTo(expectedPlaylist[i]) != 0)
				return false;
		}
...
/**
 * The objects to be sorted by the AscendingSort class. DO NOT MODIFY THIS CLASS
 * IN ANY WAY.
 */
class Song {
	 ...
	 * @param numMin the number of minutes in the duration, e.g. for a 4:23 song,
	 *               this is 4
	 * @param numSec the number of seconds in the duration, e.g. for a 4:23 song,
	 *               this is 23
	 */
	...
	/**
	 * Compare first on the basis of duration, and if the durations are the same,
	 * compare on the title.
	 * 
	 * @param s the Song to compare this to
	 * @return value less than 0 if this song is shorter/alphabetically before s,
	 *         greater than 0 if this song is longer/alphabetically after s, and 0
	 *         if they have the same title and duration.
	 */
